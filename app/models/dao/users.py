...
def check_user_duplicate(db: AsyncSession, email: str):
    db_user = db.query(User).filter(User.email == email).first()
    if db_user:
        raise HTTPException(status_code=400, detail="eamil already exists")


def get_user(db: AsyncSession, user_id: int) -> User:
...
def update_user(db: AsyncSession, user_id: int, user: UpdateUser) -> User:
    check_user_exists(db, user_id)
...
def delete_user(db: AsyncSession, user_id: int) -> None:
    check_user_exists(db, user_id)
